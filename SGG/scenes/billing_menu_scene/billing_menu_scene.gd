...
@onready var system_datetime: Dictionary = Time.get_datetime_dict_from_system()
...
func fill_dates():
	for i in range(1, 32):
		From_Day.add_item("%02d" % i)
		To_Day.add_item("%02d" % i)

	for i in range(1, 13):
		From_Month.add_item("%02d" % i)
		To_Month.add_item("%02d" % i)

	for i in range(2020, 2024):
		From_Year.add_item("%d" % i)
		To_Year.add_item("%d" % i)

	From_Day.selected = system_datetime.day - 1
	From_Month.selected = system_datetime.month - 1
	From_Year.selected = system_datetime.year - From_Year.get_item_text(0).to_int()
